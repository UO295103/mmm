...
class Simulador {
    ...
    mostrarResultado() {
        ...
        this.container.append(MensajeResultado);
        this.container.append(MensajeGanador);
    ...
    desordenarArray(arr) {
        for (let i = 2; i > 0; i--) {
            ...
            [arr[i], arr[j]] = [arr[j], arr[i]]; // Desestructuración para intercambiar
        }
        return arr;
    }
...
dibujarCarrera(resultado) {
    ...
    // Dibujar la meta como cuadros reducidos
    ...
    // Dibujar los coches (pilotos) en posiciones horizontales diferentes
    resultado.forEach((piloto, index) => {
        ...
        if (piloto.piloto === this.ganador) {
            // Nombre del ganador a la izquierda
